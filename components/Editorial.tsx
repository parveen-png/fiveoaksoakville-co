...
export function AnswerBlock({
  ...
  return (
    <div>
      <h1
        id="page-title"
        className="font-display text-4xl leading-[1.08] text-ink sm:text-5xl"
      >
        {h1}
      </h1>
      <p
...
export function Breadcrumbs({ page }: { page: PageContent }) {
  const crumbs =
    page.path === "/"
      ? [{ href: "/", name: page.breadcrumbName }]
      : [
          { href: "/", name: homePage.breadcrumbName },
          { href: page.path, name: page.breadcrumbName },
        ];
        ...
        {crumbs.map((crumb, index) => (
          <li key={crumb.href} className="flex items-center gap-2">
            ...
            {index === crumbs.length - 1 ? (
              ...
              <Link className="hover:text-bronze" href={crumb.href}>
                {crumb.name}
              </Link>
            )}
          </li>
...
export function DatedFactExcerpt({
  ...
  return (
    <aside className="border border-stone bg-paper-muted/80 p-5 md:p-6">
      ...
      {elevations.map((elevation) => {
        ...
        return (
          <article key={elevation.id}>
            ...
            <p className="mt-1 text-sm text-ink-muted">{elevation.type} · artist&apos;s concept</p>
          </article>
        );
      })}
    </div>
      ...
      {interiors.map((interior) => {
        ...
        return (
          <article key={interior.id}>
            ...
            <p className="mt-1 text-sm text-ink-muted">Artist&apos;s concept · not a floor plan</p>
          </article>
        );
      })}
    </div>
