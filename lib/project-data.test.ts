...
describe("homepage answer block", () => {
  it("keeps a 40–80 word definition that names the required entities", () => {
    const text = copy.homepageAnswer;
    ...
    expect(text).toContain("Oakville, Ontario, Canada");
    expect(text.toLowerCase()).toContain("coming-soon");
    expect(text.toLowerCase()).toContain("single-detached");
    ...
    expect(homePage.answer).toBe(copy.homepageAnswer);
  });
  ...
  it("uses a unique title versus the .ca stacked landing title", () => {
    ...
    expect(seo.title).not.toBe(
      "Five Oaks by Caivan | Dundas & Neyagawa Townhomes & Detached",
    );
...
describe("spoke answer blocks", () => {
  it("keeps unique 40–80 word answers and query-language H1s", () => {
    ...
    expect(pricingPage.title).toBe(
      "Five Oaks Prices Oakville | Official List Not Published",
    );
  });
});
...
describe("campaign images", () => {
  it("uses local files according to their source names", () => {
    ...
    expect(images.hero.alt.toLowerCase()).toContain("illustration only");
    expect(images.communityAerial.alt.toLowerCase()).toContain(
      "illustration only",
    );
    expect(images.pondPathway.alt.toLowerCase()).toContain("illustration only");
    expect(images.creekAerial.alt.toLowerCase()).toContain("illustration only");
    expect(images.lakeshore.alt.toLowerCase()).toContain("illustration only");
    expect(images.starling40.alt.toLowerCase()).toContain("artist's concept");
