...
This `.co` site is the **only indexable primary** for this campaign. It is a sibling of [fiveoaksbycaivan.ca](https://www.fiveoaksbycaivan.ca/). It uses the **same lead form contract**, **same Google Sheet**, and **same pictures**, with a **new visual design** and a hub-and-spoke URL structure.

**Canonical campaign domain:** `https://www.fiveoaksoakville.co`. Apex `fiveoaksoakville.co` 308s to www.

To keep rankings from splitting, **301 or canonical these duplicates to matching `.co` URLs**:

| Duplicate | Target |
| --- | --- |
| `https://www.fiveoaksbycaivan.ca/` | `https://www.fiveoaksoakville.co/` |
| `https://www.fiveoaksbycaivan.ca/privacy` | `https://www.fiveoaksoakville.co/privacy` |
| `https://www.fiveoaksbycaivan.ca/disclaimer` | `https://www.fiveoaksoakville.co/disclaimer` |
| `https://www.fiveoakbycaivan.com/` (if it still serves this campaign) | `https://www.fiveoaksoakville.co/` |
| Matching inner URLs on `.ca` or `fiveoakbycaivan.com` | Matching `/location`, `/homes`, `/pricing`, `/floor-plans`, `/faqs` on `.co` |

Do not index `.ca` and `.co` in parallel for the same queries.
...
1. `.env.local` is copied from the sibling `.ca` project. Public keys for this site:

```bash
NEXT_PUBLIC_SITE_URL=https://www.fiveoaksoakville.co
NEXT_PUBLIC_SITE_DOMAIN=www.fiveoaksoakville.co
NEXT_PUBLIC_LANDING_PAGE_VARIANT=co-oakville-2026-09
NEXT_PUBLIC_NOINDEX=true
```

Keep Google OAuth, spreadsheet ID, tab name, emails, analytics, verification and publisher keys as copied. Set `NEXT_PUBLIC_NOINDEX=false` only after the HTTPS www canonical is live. Do not commit `.env.local`.
...
- `/sitemap.xml` and `/sitemap/sitemap.xml` both serve the same urlset as XML with CORS `*` and without Content-Security-Policy or `X-Frame-Options`.
